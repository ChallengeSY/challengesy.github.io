...
function temporalCost(baseAmt) {
	var fragTxt = "<br /><br /><b>Cost</b>: " + baseAmt + " "+conceptLink("TP");
...
function dmBase(strength) {
	...
	if (useRuleset == "AGT") {
		dmCommonC = dmCommonC + "<br /><br />As a "+conceptLink("scenario card")+", any DMs revealed "+conceptLink("battle")+" the ship that just revealed it, even via "+conceptLink("Exploration")+". \
			...
			<br /><br /><a href=\"javascript:showBox('DMMP')\">MP</a> DMs award 2 "+conceptLink("space wreck")+" rolls instantly if destroyed.";
	}
...
function showBox(concept) {
	...
	switch (concept.toLowerCase()) {
		...
		case "competitive":
			...
			displayTxt = displayTxt + "or "+conceptLink("Team Game")+".";
			break;
		case "cp":
			headingTxt = "Construction Points (CP)";
			displayTxt = "Monetary currency. Earned by developing "+conceptLink("colonies")+", towing "+conceptLink("minerals")+", and connecting "+conceptLink("pipeline")+"s. Used to buy ";
			if (useRuleset == "AGT") {
				displayTxt = displayTxt + conceptLink("ship")+"s/"+conceptLink("grount unit")+"s, and nearly anything else that is not covered by "+conceptLink("facilities")+".";
			} else if (useRuleset != "SE4X") {
				displayTxt = displayTxt + " and support "+conceptLink("ship")+"s/"+conceptLink("grount unit")+"s, place "+conceptLink("bid")+"s, and (if not playing with "+conceptLink("Research Center")+"s) develop "+conceptLink("technologies")+".";
			} else {
				displayTxt = displayTxt + " and support "+conceptLink("ship")+"s, place "+conceptLink("bid")+"s, and develop "+conceptLink("technologies")+".";
			}
			break;
		case "co-op":
			// Fall thru
		...
		case "exploration":
			...
			if (useRuleset == "talon") {
				...
				if (useRuleset != "SE4X") {
					displayTxt = displayTxt + "<br />Level 2 allows "+conceptLink("Base")+"s / "+conceptLink("Ship Yard")+"s / Exploration 1 ships";
					if (useRuleset != "CE") {
						displayTxt = displayTxt + " / "+conceptLink("Battle Carrier")+"s";
						if (useRuleset == "AGT") {
							displayTxt = displayTxt + " / "+conceptLink("AP Bot")+"s";
						}
					}
					displayTxt = displayTxt + " to respond to \
		...
		case "immobile":
			...
			if (useRuleset == "talon") {
				...
				if (useRuleset == "SE4X" || useRuleset == "CE") {
					...
					displayTxt = displayTxt + conceptLink("Ship Yard")+"s and "+conceptLink("Base")+"s require the "+conceptLink("empire advantage")+" "+conceptLink("On the Move")+" to have <i>any</i> mobility.";
					if (useRuleset == "AGT") {
		...
		case "maintenance":
			...
			if (useRuleset != "SE4X") {
				...
				if (useRuleset != "CE") {
					displayTxt = displayTxt + ", "+conceptLink("Replicators") + ", "+conceptLink("Pirate")+"s";
				}
			}
			displayTxt = displayTxt + " require no maintenance.)<br />If maintenance exceeds total income, then net income is set to zero.";
			if (useRuleset == "AGT") {
				displayTxt = displayTxt + "<br /><br />Since "+conceptLink("AP Bot")+"s use "+conceptLink("economic roll")+"s to generate CP, they instead roll one less "+conceptLink("home system")+" die \
		...
		case "movement":
			...
			if (useRuleset != "talon") {
				displayTxt = displayTxt + "<br />As a "+conceptLink("technology")+", improving this increases the number of total "+conceptLink("hex")+"es that most "+conceptLink("ship")+"s can move per "+conceptLink("economic phase")+".\
					...
					<table><tr><th>Level</th><th>Turn 1</th><th>Turn 2</th><th>Turn 3</th><th>Total</th></tr>\
					<tr class=\"numeric\"><td>1</td><td>1</td><td>1</td><td>1</td><td>3</td></tr>\
					<tr class=\"numeric\"><td>2</td><td>1</td><td>1</td><td>2</td><td>4</td></tr>\
					<tr class=\"numeric\"><td>3</td><td>1</td><td>2</td><td>2</td><td>5</td></tr>\
					<tr class=\"numeric\"><td>4</td><td>2</td><td>2</td><td>2</td><td>6</td></tr>\
					<tr class=\"numeric\"><td>5</td><td>2</td><td>2</td><td>3</td><td>7</td></tr>\
					<tr class=\"numeric\"><td>6</td><td>2</td><td>3</td><td>3</td><td>8</td></tr>\
					<tr class=\"numeric\"><td>7</td><td>3</td><td>3</td><td>3</td><td>9</td></tr></table>";
			}
			break;
		case "mvp":
			headingTxt = "Most Valuable Player (MVP)";
			displayTxt = "The element that was/is the most influential towards the outcome of a given game.";
			break;
		...
		case "non-player alien":
			...
			if (useRuleset == "SE4X") {
				displayTxt = displayTxt + "If any "+conceptLink("ship")+"s are found, they must be defeated";
			} else {
		...
		case "public information":
			...
			if (useRuleset == "AGT") {
				displayTxt = displayTxt + " <span class=\"bindTxt\">The "+conceptLink("Hidden Exploration")+"</span> variant introduces an exception towards "+conceptLink("deep space")+".";
			}
			break;
		case "retreat":
			if (useRuleset == "talon") {
				displayTxt = "Process by which a "+conceptLink("ship")+" removes itself from "+conceptLink("battle")+" alive. Can be performed by entering a "+conceptLink("worm hole")+"; \
					or by punching the FTL drive on the very last step of a "+conceptLink("power phase")+", provided the ship has been "+conceptLink("spool")+"ed.";
			} else {
		...
		case "starting forces":
			displayTxt = "The ships (and other assets) that a player starts with, before the first ";
			if (useRuleset == "talon") {
				displayTxt = displayTxt + conceptLink("impulse")+" even takes place.";
			} else {
				displayTxt = displayTxt + conceptLink("turn")+" even takes place, and before applying "+conceptLink("Quick Start")+".<br />Unless specified otherwise, \
					each player starts with a "+conceptLink("homeworld")+", 4 "+conceptLink("Ship Yard")+"s, 3 "+conceptLink("scout")+"s, 3 "+conceptLink("colony ship")+"s, 1 "+conceptLink("miner");
				if (useRuleset != "SE4X") {
					displayTxt = displayTxt + ", and optionally 1 "+conceptLink("flagship");
					if (useRuleset != "CE") {
						displayTxt = displayTxt + "<br /><br />Since the "+conceptLink("replicators")+" have a different set of ships, \
							they instead start with a "+conceptLink("homeworld")+", 5 "+conceptLink("Type 0")+"s and 1 "+conceptLink("flagship");
					}
				}
			}
			break;
		...
		case "space imperia 4x":
			...
			break;
		case "team game":
			displayTxt = "A format where players may belong to a team. Teams could be "+conceptLink("Uneasy Alliance")+"s or "+conceptLink("Blood Brothers")+". Teams share in the accomplishments of "+conceptLink("primary objective")+"s.";
			break;
		case "movement turn":
			// Fall through
		case "turn":
		...
		case "cloaking":
			...
			displayTxt = "Allows building "+conceptLink("Raider")+"s. They cloak by default (able to gain "+conceptLink("weapon class")+" A), \
				but can be detected by "+conceptLink("Scanning")+" or nullified by terrain.<br />\
				...
				Level 2 equipment adds an additional Attack +1 (to Raiders) throughout the battle and negates Scanning 1.";
			break;
		case "minelaying":
		...
		case "scanning":
			...
			displayTxt = conceptLink("Destroyer")+"s equipped with this "+conceptLink("technology")+" will \
				detect "+conceptLink("Raider")+"s (and other cloaked ships) with an equal or lower "+conceptLink("cloaking")+" level.";
			if (useRuleset != "SE4X" && useRuleset != "CE" && useRuleset != "talon") {
				...
				if (useRuleset == "AGT") {
					displayTxt = displayTxt + "<br /><br />As a "+conceptLink("deep space planet attribute")+", this perk allows its owner to reveal <i>all</i> enemy ships within 2 "+conceptLink("hex")+"es of the "+conceptLink("colony")+"."+dspaStats(6, 2);
				}
			}
			break;
		case "ship size":
		...
		case "upgrade":
			headingTxt = "Upgrade";
			displayTxt = "The process of upgrading "+conceptLink("technology")+", usually on a "+conceptLink("ship")+". This usually is <i>not</i> an automatic process. \
				Outdated <q>equipment</q> can be upgraded at a "+conceptLink("Ship Yard")+", at a cost of "+conceptLink("CP")+" equal to its "+conceptLink("Hull Size")+". \
				The ship group must not move for an entire "+conceptLink("turn")+" to perform their upgrades.<br />\
				(Exception: "+conceptLink("Base")+"s, "+conceptLink("Decoy")+"s, and "+conceptLink("Ship Yard")+"s are always exceptions to the manual process.)";
			break;
		...
		case "asteroid":
			...
			if (useRuleset == "talon") {
				displayTxt = "Damages any ship (except "+conceptLink("fighter")+"s) that enters the "+conceptLink("hex")+". \
					Reduces accuracy of any non-missile weapon fire that passes through. Can block "+conceptLink("power")+" transmission.";
			} else {
		...
		case "black hole":
			if (useRuleset == "talon") {
				displayTxt = "Sucks in "+conceptLink("ship")+"s one "+conceptLink("hex")+" closer in between "+conceptLink("round")+"s, instantly destroying any that enter them. \
					Reduces accuracy of non-missile weapon fire against any targets that are closer to this hex, relative to the source ship.";
			} else {
				displayTxt = "Forces each "+conceptLink("ship")+" (except cargo) that enters to roll a survival d10 roll, \
					unless they follow a "+conceptLink("Pipeline")+" network.<br />A roll of &ge;7 causes the ship to be destroyed.<br /><br />\
					"+conceptLink("Fighter")+"s and "+conceptLink("ground unit")+"s in cargo are only destroyed if in excess of capacity of surviving ships.";
			}
			break;
		case "colonies":
			// Fall through
			...
			if (useRuleset != "talon") {
				...
				if (useRuleset != "SE4X" && useRuleset != "CE") {
					...
					displayTxt = displayTxt + "<br /><br />"+conceptLink("Replicator")+" colonies do not produce CP. Instead, they build "+conceptLink("hull")+"s locally once at full strength.\
						<br />They receive 10 CP for each "+conceptLink("alien technology")+" card they would otherwise keep when colonizing "+conceptLink("NPA")+" planets.";
				}
			}
			break;
		case "danger":
		...
		case "nebula":
			if (useRuleset == "talon") {
				displayTxt = "Prevents use of "+conceptLink("shield")+"s, and negates reinforcements. Reduces accuracy of passing non-missile weapon fire. \
		...
		case "planet":
			...
			if (useRuleset == "talon") {
				displayTxt = displayTxt + "Can block weapon fire and "+conceptLink("power")+" transmission.<br />\
					Instantly destroys any non-"+conceptLink("transport")+" ship that enters its "+conceptLink("hex")+". Invulnerable, unless specified by scenario.";
			} else {
		...
		case "decoy":
			...
			break;
		case "ship yard":
			displayTxt = conceptLink("Immobile")+" Space station able to build more "+conceptLink("ship")+"s. Can be built at any "+conceptLink("colony")+" that has produced income this "+conceptLink("economic phase")+
		...
		case "scout":
			...
			if (useRuleset == "talon") {
				...
				displayTxt = displayTxt + statsTalon("Terran", 37, "Phaser", "2/1/1/1", 2, null, 1);
			} else {
		...
		case "base":
			...
			if (useRuleset == "talon") {
				...
				displayTxt = displayTxt + " with powerful long range weaponry. \
					One can be built at any "+conceptLink("colony")+" that has produced income this "+conceptLink("economic phase");
				if (useRuleset == "rep" || useRuleset == "AGT") {
					displayTxt = displayTxt + "<br />\
		...
		case "battlecruiser":
			...
			if (useRuleset == "talon") {
				...
				displayTxt = displayTxt + statsTalon("Talon", 142, "Dual Disruptors + Disruptor + Triple Missile Launchers", "8/4/4/4", 7, "3/5", "2/4");
			} else {
		...
		case "battleship":
			...
			if (useRuleset == "talon") {
				...
				displayTxt = displayTxt + statsTalon("AI", 200, "Laser + Cobalt Cannon x2", "9/9/9/9", 9, 5, 5);
			} else {
		...
		case "carrier":
			...
			if (useRuleset == "talon") {
				...
				displayTxt = "Combat transport craft, able to carry up to 3 "+conceptLink("Fighter")+"s into space, and to launch them into "+conceptLink("battle")+".";
		...
		case "fighter":
			if (useRuleset == "talon") {
				...
				if (useRuleset == "AGT") {
					displayTxt = displayTxt + "<br /><br />The Alternate craft does not require a carrier, and <i>all</i> B6 / B7 variants get "+conceptLink("Defense")+" +1 versus "+conceptLink("Point-Defense")+".";
				}
		...
		case "pipeline":
			...
			break;
		case "raider":
			displayTxt = "Sneaky "+conceptLink("combat ship")+", benefits from cloaking (A)";
			displayTxt = displayTxt + stats4X("Base", 12, "D4", 0, 2, -1, conceptLink("Cloaking")+" 1");
			displayTxt = displayTxt + stats4X("Alternate", 14, "D5", 0, 2, -1, conceptLink("Cloaking")+" 1");
			break;
		case "sw":
			// Fall through
		case "science vessel":
		...
		case "alien-d":
			...
			displayTxt = "Light "+conceptLink("non-player alien")+" ship, equipped with "+conceptLink("Point-Defense")+" 2";
			displayTxt = displayTxt + stats4X("Alien", 0, "D4", 1, 1, 1);
			break;
		case "alien-c":
			headingTxt = "Alien Cruiser";
			displayTxt = "Medium "+conceptLink("non-player alien")+" ship, equipped with "+conceptLink("Point-Defense")+" 2";
			displayTxt = displayTxt + stats4X("Alien", 0, "C5", 2, 1, 1);
			break;
		case "alien-b":
			headingTxt = "Alien Battlecruiser";
			displayTxt = "Heavy "+conceptLink("non-player alien")+" ship, equipped with "+conceptLink("Point-Defense")+" 2";
			displayTxt = displayTxt + stats4X("Alien", 0, "B6", 2, 1, 1);
			break;
		case "alien-e":
			headingTxt = "Alien Frigate";
			displayTxt = "Armored Light "+conceptLink("non-player alien")+" ship, equipped with "+conceptLink("Scanning")+" 1";
			displayTxt = displayTxt + stats4X("Alien", 0, "E7", 1, 2, 1);
			break;
		case "alien-a":
			headingTxt = "Alien Battleship";
			displayTxt = "Armored Heavy "+conceptLink("non-player alien")+" ship, equipped with "+conceptLink("Scanning")+" 1";
		...
		case "expansion fleet":
			if (useRuleset == "AGT") {
				displayTxt = conceptLink("Alien Player")+" or "+conceptLink("AP Bot")+" "+conceptLink("fleet")+" that focuses \
					on assaulting "+conceptLink("planet")+"s, usually in an effort to "+conceptLink("capture")+" (if able).<br />\
					If fleet strength is at 60 "+conceptLink("CP")+" or less, <i>or</i> this is bound to a regular "+conceptLink("Alien Player")+", \
					the fleet becomes a "+conceptLink("guard fleet")+" after conquering the planet.";
			} else {
				displayTxt = conceptLink("Alien Player")+" "+conceptLink("fleet")+" that focuses on assaulting "+conceptLink("planet")+"s, \
					usually in an effort to "+conceptLink("capture")+" (if able). Once the planet is conquered, this fleet acts as a "+conceptLink("guard fleet")+".";
			}
			break;
		...
		case "close encounters":
			displayTxt = "First expansion to "+conceptLink("Space Empires 4X")+".<br />\
				...
				"+conceptLink("facilities")+", "+conceptLink("empire advantage")+"s, "+conceptLink("co-op")+", and "+conceptLink("space amoeba")+".";
			break;
		case "boarding":
		...
		case "capture":
			...
			displayTxt = conceptLink("Combat Ship")+"s and "+conceptLink("planet")+"s can be captured in the respective "+conceptLink("battle")+"s. \
				...
				<br /><br />"+conceptLink("Boarding Ship")+"s are used to capture opposing ships. Ships captured this way are highly vulnerable until 2 "+conceptLink("round")+"s following capture.\
				<br />(Does not count towards fleet size, nor can it shoot/"+conceptLink("retreat")+" in this state.)\
				<br /><br />"+conceptLink("Ground Unit")+"s are used to capture opposing planets. Planets captured this way are reduced as if they endured 1 "+conceptLink("bombardment")+" hit, but survive being reduced to 0. (A colony already at 0 is instead treated as if the victor just colonized the planet.)";
			break;
		case "experience":
		...
		case "security forces":
			...
			break;
		case "titan":
			displayTxt = "Extremely potent <q>baseship</q>. Can carry/launch 3 "+conceptLink("Fighter")+"s if upgraded.\
				...
				displayTxt = displayTxt + stats4X("Alternate", "&infin;", "A7", 3, 5);
				if (useRuleset == "AGT") {
					displayTxt = displayTxt + "<br /><b>Required <a href=\"javascript:showBox('Empire Advantage')\">Advantage</a></b>: "+conceptLink("War Sun");
				}
			break;
			...
			if (useRuleset == "talon") {
				...
				if (useRuleset == "AGT") {
					displayTxt = "Utility "+conceptLink("combat ship")+", able to pick up 6 "+conceptLink("ground unit")+"s and/or "+conceptLink("fighter")+"s; or up to 30 "+conceptLink("LP")+" worth of crates\
						<br />Fighters carried this way can not be launched into "+conceptLink("battle")+"";
				} else {
		...
		case "sa3":
			...
			break;
		case "sa4":
			displayTxt = amoebaBase(4) + stats4X("Boss", 0, "A7 / A5", "2 / 1", 2, 2)+"<br /><b>Special</b>: Stats reduced when in "+conceptLink("battle")+" with only "+conceptLink("Raider")+"s";
			break;
		case "sa5":
			displayTxt = amoebaBase(5) + stats4X("Boss", 0, "B5", 1, 2, 2)+"<br /><b>Special</b>: Starts with <i>2</i> "+conceptLink("swallow")+" attempts per "+conceptLink("round");
			break;
		case "sa6":
			// Fall through
		case "sa7":
		...
		case "minesweep jammer":
			displayTxt = conceptLink("Minesweeper")+"s used against the holder are one level less in effectiveness. Reducible to level 0 (0 mines per sweeper).\
				<br /><br />"+conceptLink("Replicator")+" "+conceptLink("Type SW")+"s used against the holder are instead half as effective (round down <i>totals</i> per "+conceptLink("hex")+").";
			break;
		case "air support":
		...
		case "advance shipyards":
			...
			break;
		case "lorelei system":
			displayTxt = "Allows the user to override the defender's choice on "+conceptLink("minelayer")+" usage, allowing the user's "+conceptLink("Scout")+"s and "+conceptLink("Destroyer")+"s to be targeted.";
			break;
		...
		case "space pilgrims":
			displayTxt = "Empire's ships are unaffected by "+conceptLink("Asteroid")+"s, "+conceptLink("Nebula")+", \
				"+conceptLink("Black Hole")+"s, and "+conceptLink("Lost in Space")+" markers when moving; even when using a "+conceptLink("slingshot")+".";
			if (useRuleset == "AGT") {
				displayTxt = displayTxt + "<br /><br />If acquired as an "+conceptLink("auxiliary")+" technology, this perk affects <i>only</i> the ship type the tech was drawn for.";
			}
			break;
		case "hive mind":
		...
		case "gifted scientists":
			displayTxt = "Technologies cost 33% less points, at the expense of <i>all</i> ships costing 1 more "+conceptLink("CP")+" to build.";
			break;
		...
		case "expert tacticians":
			displayTxt = "Empire gains "+conceptLink("Fleet Size Bonus")+" if they outnumber their opponent at all. Opponents do not gain FSB until they outnumber this empire 3:1.\
				<br /><br />If both fleets have access to this benefit, they negate each other.";
			break;
		case "horsemen of the plains":
			headingTxt = "Horsemen of the Plains";
		...
		case "powerful psychics":
			displayTxt = "Empire starts with "+conceptLink("Exploration")+" 1. Additionally, Exploration equipped ships can remote reveal \
				<i>all</i> adjacent reveal enemy stacks (but not tech), as if those enemies were sent to "+conceptLink("battle")+".";
			break;
		case "shape shifters":
			if (useRuleset == "AGT") {
				displayTxt = "Empire can build "+conceptLink("Decoy")+"s at any "+conceptLink("hex")+" with friendly "+conceptLink("combat ship")+"s or "+conceptLink("colony")+" \
					...
					They also get a single use of "+conceptLink("Retreat When Engaged")+".";
			} else {
		...
		case "war sun":
			displayTxt = "Empire starts with 1 "+conceptLink("Titan")+". This starting ship pays no "+conceptLink("maintenance")+" while at a friendly "+conceptLink("colony")+". \
				It may not leave their owner's "+conceptLink("home system")+"s until "+conceptLink("Ship Size")+" 6. Empire's ships can cross (but not end a move in) "+conceptLink("supernova")+"s.";
			break;
		...
		case "anti-sensor hull":
			...
			if (useRuleset == "AGT") {
				displayTxt = displayTxt + "<br />"+conceptLink("Battle Carrier")+"s and potentially a random alternate type (as granted by "+conceptLink("auxiliary")+" tech) are also equipped with this perk.";
			} else if (useRuleset == "rep") {
		...
		case "design weakness":
			...
			if (useRuleset == "AGT") {
				displayTxt = displayTxt + "<br /><br />This perk works differently if activated via "+conceptLink("Know the Weakness")+".";
			}
			break;
		...
		case "pirate":
			...
			displayTxt = "Non-aligned ship that can be fought or hired by any ships that encounter them. If fought, the ship disappears once the "+conceptLink("battle")+" is over.<br />\
				Has fixed "+conceptLink("Movement")+" 4 and "+conceptLink("Fastmove")+" and Skilled "+conceptLink("experience")+". Automatically "+conceptLink("self-destruct")+"s when "+conceptLink("capture")+"d.<br />\
				May neither be given additional equipment, nor may it enter "+conceptLink("unexplored")+" terrain, nor may it enter a battle except when defending or alone.\
		...
		case "battle carrier":
			...
			displayTxt = "Heavy "+conceptLink("Carrier")+" variant, equipped with "+conceptLink("Anti-Sensor Hull")+"s, \
				and able to carry/launch 6 "+conceptLink("Fighter")+"s and benefit from "+conceptLink("Fastmove")+" 2 technology\
		...
		case "quick study":
			displayTxt = "(Play before any group rolls for "+conceptLink("experience")+")<br />\
				Shift the dice rolls for that group by +2 or -2 (user's choice).<br /><br />\
				In a "+conceptLink("Replicator")+" hand, this card is instead "+conceptLink("Extra Move")+"." + discardVal(3,1);
			break;
		case "discover member of ancient race":
			...
			displayTxt = "(Play at start of "+conceptLink("economic phase")+" involving a "+conceptLink("space wreck")+")<br />\
				After rolling to cash in one of their "+conceptLink("space wreck")+"s, the user may shift by up to 2 (in direction of user's choice), wrapping around as necessary.<br /><br />\
		...
		case "overload weapons":
			displayTxt = "(Play with one additional discard; before one of the ships with "+conceptLink("Attack")+" "+conceptLink("tech")+" 1+ shoots)<br />\
				...
				<br />In return, the ship's final Attack strength is reduced to <b>0</b> <i>after</i> it fires its overloaded ammo.<br /><br />\
		...
		case "unconventional boarding":
			displayTxt = "(Play with one additional discard; before one of the ships <q>shoots</q>)<br />\
				...
				Each "+conceptLink("Boarding")+" level adds +2 to strength. Only usable against non-immune targets.";
			if (useRuleset == "AGT") {
				displayTxt = displayTxt + "<br />Neither "+conceptLink("Immortals")+" nor alternate "+conceptLink("faction")+" can play this card, \
					nor can the card be used against the latter.";
			} else {
				displayTxt = displayTxt + " "+conceptLink("Immortals")+" can not play this card.";
			}
			displayTxt = displayTxt + "<br /><br />In a "+conceptLink("Replicator")+" hand, this card is instead "+conceptLink("Extra Move")+"." + discardVal(5,3);
			break;
		...
		case "deep cover operative":
			displayTxt = "(Play at start of user's "+conceptLink("turn")+" or "+conceptLink("economic phase")+")<br />\
				Take one face up non-mission "+conceptLink("resource card")+" from target opponent, and add it to the user's hand. This does not mess with existing effects." + discardVal(4);
			break;
		case "forced system shutdown":
			displayTxt = "(Play in betweeen "+conceptLink("battle")+" setup and "+conceptLink("round")+" 1)<br />\
				One enemy ship has its "+conceptLink("Attack")+"/"+conceptLink("Defense")+"/"+conceptLink("Tactics")+" equipment reduced to 0 until "+conceptLink("round")+" 4.\
		...
		case "coup":
			displayTxt = "(Play after conclusion of the last "+conceptLink("battle")+" of <i>any</i> "+conceptLink("hex")+")<br />\
				...
				<br /><br />If the chosen empire is a "+conceptLink("Replicator")+" empire, then the victor's homeworld instead produces 50% more CP (round down)." + discardVal(4,2);
			break;
		...
		case "splash damage":
			displayTxt = "(Play when a "+conceptLink("combat ship")+" gets destroyed in "+conceptLink("battle")+")<br />\
				...
				Each ship targeted this way rolls for evasion (&ge;6 to avoid damage). Ships destroyed this way award no "+conceptLink("experience")+"." + discardVal(2);
			break;
		...
		case "depletion":
			...
			displayTxt = conceptLink("Replicator")+" "+conceptLink("colonies")+" grow/produce quickly, but also quickly consume their "+conceptLink("planet")+"s' resources.\
				<br />Each "+conceptLink("economic phase")+" (starting on EP 10 by default), the closest colony is depleted (prioritizing full strength);\
				<br />rendering its planet entirely useless for the rest of the scenario.\
		...
		case "hull":
			if (useRuleset == "talon") {
				displayTxt = "The inner armor that makes up a "+conceptLink("ship")+". If its hull armor gets reduced to 0, the ship "+conceptLink("explode")+"s. The hull capacity also plays a role in "+conceptLink("spool")+"ing.<br /><br />\
					"+conceptLink("Critical")+" damage may be sustained along the way, and "+conceptLink("power")+" output may be reduced.";
			} else {
		...
		case "advanced munitions":
			displayTxt = "Ability that allows the compatible ship to mount one additional "+conceptLink("Attack")+" technology, allowing even Attack 4. \
				Available exclusively as "+conceptLink("auxiliary")+" technology.";
			break;
		case "construction efficiency":
			displayTxt = "Compatible ships cost 2 less "+conceptLink("CP")+" to build. Available exclusively as "+conceptLink("auxiliary")+" technology.";
			break;
		case "storm movement":
			// Fall thru
		case "cosmic storm":
			...
			displayTxt = "Unique terrain that can move 1 "+conceptLink("hex")+" in a random direction each "+conceptLink("economic phase")+". \
				Breaks apart if it tries to enter a "+conceptLink("black hole")+".<br />\
		...
		case "lp":
			...
			if (useRuleset == "talon") {
				...
				displayTxt = "Currency that is used exclusively to pay for "+conceptLink("maintenance")+" or to "+conceptLink("bid")+". \
					...
					"+conceptLink("CP")+" can be converted to LP as necessary to replace any shortage, but this process is extremely inefficient. \
		...
		case "missile":
			if (useRuleset == "talon") {
				displayTxt = conceptLink("Talon")+" seeker that homes in on its victim. Deals 2 damage when connected, but can be shot down. Has a fixed "+conceptLink("power curve")+" of <u>0-6-0</u>.<br />\
					 Intact missiles require 3 damage to destroy in one hit. Damaged missiles only require 1 more damage to finish.";
			} else {
		...
		case "space station":
			displayTxt = "Non-aligned base that fights any ships in the same "+conceptLink("hex")+". \
				...
				<br />The first "+conceptLink("faction")+" to defeat them receives the reward indicated on its counter.";
		...
		case "starbase":
			...
			if (useRuleset == "talon") {
				...
				displayTxt = displayTxt + " with overpowering weaponry, can shoot twice per "+conceptLink("round")+"! Can not move. \
					Requires upgrading an Advanced "+conceptLink("Base")+" at a "+conceptLink("colony")+" that has produced maximum natural income. \
		...
		case "supply range":
			displayTxt = conceptLink("All Good Things")+" technology that determines how many "+conceptLink("hex")+"es a "+conceptLink("combat ship")+" \
				...
				The starting range is 1 hex, but can be upgraded to up to 4 hexes away. \
				Only effective when "+conceptLink("Logistic Center")+"s are enabled.<br /><br />\
				Tracing is required; can pass into (but not through) "+conceptLink("supernova")+"s, "+conceptLink("quantum filament")+"s, \
		...
		case "temporal engine":
			displayTxt = "Experimental engines that can facilitate "+conceptLink("temporal effect")+"s.<br /><br />\
				"+conceptLink("Base")+"s (+ their upgrades) and "+conceptLink("Ship Yard")+"s are equipped with these, as is one of the following random ship types (determined for ALL players):\
					<ul style=\"max-width: 165px; margin-top: 0px; margin-bottom: 0px; margin-left: auto; margin-right: auto; text-align: left;\">\
					<li class=\"noKeywords\">"+conceptLink("Minesweeper")+"</li>\
					<li class=\"noKeywords\">"+conceptLink("Transport")+"</li>\
					<li class=\"noKeywords\">"+conceptLink("Scout")+"</li>\
					<li class=\"noKeywords\">"+conceptLink("Destroyer")+"</li>\
					<li class=\"noKeywords\">"+conceptLink("Carrier")+" / "+conceptLink("Missile Boat")+"</li>\
					<li class=\"noKeywords\">"+conceptLink("Cruiser")+"</li></ul>";
			break;
		case "temporal effect":
			displayTxt = "Effects that arise when spending "+conceptLink("TP")+" to mess with the space time continuum.<br /><br />\
				Some effects are limited to the same "+conceptLink("hex")+" as a "+conceptLink("temporal engine")+", others are usable from 3 hexes away, and others can be used anywhere as long as one exists.";
			break;
		case "crossing the event horizon":
			headingTxt = "Crossing the Event Horizon";
			displayTxt = "Ships accompanying a "+conceptLink("temporal engine")+" have +40% to surviving a "+conceptLink("black hole")+" check (raises it to 100%, unless using a "+conceptLink("slingshot")+")." + temporalCost("10 / unit");
			break;
		case "redline the engines":
			headingTxt = "Redline the Engines";
			displayTxt = "Ships accompanying a "+conceptLink("temporal engine")+" may elect to move an additional "+conceptLink("hex")+" this "+conceptLink("turn")+"." + temporalCost("15 / unit");
			break;
		case "reroute targeting computers":
			displayTxt = "Reroll up to 4 dice from any "+conceptLink("combat ship")+" group, against the same targets in a battle involving "+conceptLink("temporal engine")+"(s)." + temporalCost(10);
			break;
		case "temporal maneuver":
			displayTxt = "At the start of a space "+conceptLink("battle")+" (after "+conceptLink("minelayer")+"s are done), the player moves one of their ships (able to move, but without a "+conceptLink("temporal engine")+") and its cargo to this battle.<br /><br />\
				OR, at the start of a ground "+conceptLink("battle")+", the player moves one of their ground units to this battle. Ground units moved this way count as 1 Hull Point for cost purposes." + temporalCost("10 / Hull Point (minimum 10)");
			break;
		case "focused production":
			displayTxt = "A "+conceptLink("Ship Yard")+" may be relocated from anywhere to a "+conceptLink("temporal engine")+" that is orbiting a friendly "+conceptLink("colony")+"." + temporalCost(25);
			break;
		case "suspension of disbelief":
			headingTxt = "Suspension of Disbelief";
			displayTxt = "If a "+conceptLink("temporal engine")+" survives a "+conceptLink("battle")+", it may save a "+conceptLink("crew card")+" that just died." + temporalCost(20);
			break;
		case "end of an empire":
			headingTxt = "End of an Empire";
			displayTxt = "If a "+conceptLink("temporal engine")+" is in the same "+conceptLink("hex")+" as an enemy "+conceptLink("colony")+", it may instantly eliminate that player." + temporalCost(350);
			break;
		case "long range scan":
			displayTxt = "Target group within 3 "+conceptLink("hex")+"es of a "+conceptLink("temporal engine")+" is revealed." + temporalCost(15);
			break;
		case "engine sabotage":
			displayTxt = "Target revealed ship within 3 "+conceptLink("hex")+"es of a "+conceptLink("temporal engine")+" is immobilized on its next "+conceptLink("turn")+"." + temporalCost("10 / unit");
			break;
		case "recharting the stars":
			headingTxt = "Recharting the Stars";
			displayTxt = "Target "+conceptLink("hex")+" within 3 hexes of a "+conceptLink("temporal engine")+" gains a fresh "+conceptLink("deep space")+" marker, provided it is adjacent to another "+conceptLink("unexplored")+" deep space hex." + temporalCost(30);
			break;
		case "aid aliens":
			displayTxt = "Target "+conceptLink("planet")+" that already has at least one "+conceptLink("NPA")+" ship gains an extra ship." + temporalCost(10);
			break;
		case "uncharted corruption":
			displayTxt = "Randomly discard one "+conceptLink("resource card")+" from target opponent's hand." + temporalCost(15);
			break;
		case "rapid research":
			displayTxt = "Two levels of any single "+conceptLink("technology")+" can be bought in a single "+conceptLink("economic phase")+"." + temporalCost(30);
			break;
		...
		case "exploration vessel":
			...
			displayTxt = conceptLink("AP Bot")+"-exclusive ship that is compatible with "+conceptLink("Exploration")+" 1. Appears on the board as a "+conceptLink("Miner-X")+".";
		...
		case "forward operating base":
			...
			displayTxt = conceptLink("AP Bot")+"-exclusive base that allows launching "+conceptLink("fleet")+"s at a non-"+conceptLink("homeworld")+" "+conceptLink("hex")+". Appears on the board as a "+conceptLink("Ship Yard")+".\
				<br /><br />The first time this appears in battle, a "+conceptLink("Base")+" is automatically built, using included materials. "+conceptLink("Satellites")+" may also be bought.";
		...
		case "guard fleet":
			...
			break;
		case "morale":
			displayTxt = "A level that an "+conceptLink("AP Bot")+" uses to measure its confidence in "+conceptLink("battle")+". \
				...
				<br /><br />The threshold is set to the lowest stage each "+conceptLink("round")+", and modifiers are then applied as follows:\
				<ul style=\"max-width: 450px; margin-top: 0px; margin-left: auto; margin-right: auto; text-align: left;\">\
				<li class=\"noKeywords\">+1 stage for each damaged AP ship</li>\
				<li class=\"noKeywords\">-1 stage for each damaged player ship</li>\
				<li class=\"noKeywords\">+1 stage if the opposition has uncountered technology</li>\
				<li class=\"noKeywords\">+1 stage if more than half the fleet is damaged/destroyed</li>\
				<li class=\"noKeywords\">+2 stages for each AP ship lost in battle</li>\
		...
		case "paranoia":
			displayTxt = "A level that an "+conceptLink("AP Bot")+" uses to measure its confidence in its current "+conceptLink("fleet strategy")+". \
				...
				<br /><br />Generally, paranoia is influenced by "+conceptLink("battle")+"s and by "+conceptLink("colony")+" destruction/"+conceptLink("capture")+" against a human player. It is reset whenever the strategy is changed.";
			displayTxt = displayTxt + paranoiaTable();
			break;
		...
		case "random encounter":
			displayTxt = "A system where the "+conceptLink("AP Bot")+" may ambush any player ships that move in their "+conceptLink("home system")+"s.<br /><br />\
				If the roll is successful (&ge;9), one random "+conceptLink("hex")+" (from among those closest to their "+conceptLink("homeworld")+") is chosen to have &approx;30 "+conceptLink("CP")+" worth of ships appear out of nowhere to conduct a "+conceptLink("battle")+". After conclusion, any survivors make it directly back into the fleet bank.";
			break;
		case "shattered fleet":
			...
			break;
		case "shell game":
			displayTxt = "Strategy used by a <a href=\"javascript:showBox('Hidden Fleet')\">hidden</a> "+conceptLink("extermination fleet")+" to deploy multiple <q>fleets</q> across different routes, with only one of them being the real fleet. <span class=\"bindTxt\">(The rest are "+conceptLink("decoy")+"s.)</span>";
			break;
		...
		case "key ship":
			...
			break;
		case "biggest ship":
			headingTxt = "Biggest Ships";
			displayTxt = conceptLink("Fleet strategy")+" that focuses on the largest "+conceptLink("Ship Size")+"s attainable. It builds the largest ships available with its "+conceptLink("technology")+" and assigned "+conceptLink("CP")+".";
			break;
		case "buffed cruisers":
			...
			break;
		case "hit and run":
			headingTxt = "Hit and Run";
			displayTxt = conceptLink("Fleet strategy")+" that prioritizes "+conceptLink("Battlecruiser")+" and "+conceptLink("Fastmove")+" tech.";
			break;
		case "scout rush":
			displayTxt = conceptLink("Fleet strategy")+" that builds "+conceptLink("Scout")+"s exclusively.";
			break;
		case "yarr":
			headingTxt = "Yarr!";
		...
		case "spice":
			...
			break;
		case "organia":
			displayTxt = "This "+conceptLink("planet")+" can neither be colonized nor "+conceptLink("capture")+"d nor connected to a "+conceptLink("pipeline")+" network. \
				Prohibits "+conceptLink("battle")+"s from taking place."+dspaStats(0, 0);
			break;
		case "dilithium crystals":
			displayTxt = "A fully developed "+conceptLink("colony")+" produces a dilithium counter (if there is not already one present), \
				which can be picked up by a "+conceptLink("transport")+" and delivered to a "+conceptLink("homeworld")+" to gain "+conceptLink("CP")+". \
				The exact amount is determined by the shortest route between the two planets.\
				<br /><br />Removed if a "+conceptLink("replicator")+" player fully develops a colony, but they gain 10 CP in return."+dspaStats(5, 2);
			break;
		case "abundant":
			displayTxt = "Produces 2 extra "+conceptLink("CP")+" per "+conceptLink("economic phase")+" while colonzed, even for a "+conceptLink("replicator")+" player.\
				<br /><br />"+conceptLink("AP Bot")+" colonies with this attribute instead produce an extra "+conceptLink("economic roll")+" each fifth "+conceptLink("economic phase")+".\
				"+dspaStats(5, 2);
			break;
		case "wealthy":
			displayTxt = "Produces 1 extra "+conceptLink("CP")+" per "+conceptLink("economic phase")+" while colonzed. Removed if colonized by a "+conceptLink("replicator")+" player.\
				<br /><br />"+conceptLink("AP Bot")+" colonies with this attribute instead produce an extra "+conceptLink("economic roll")+" each tenth "+conceptLink("economic phase")+".\
				"+dspaStats(4, 1);
			break;
		case "poor":
			displayTxt = "Produces 1 less "+conceptLink("CP")+" per "+conceptLink("economic phase")+" while colonzed (minimum 0). Removed if colonized by a "+conceptLink("replicator")+" player.\
				<br /><br />"+conceptLink("AP Bot")+" colonies with this attribute instead produce <i>no</i> "+conceptLink("economic roll")+" each tenth "+conceptLink("economic phase")+".\
				"+dspaStats(2, 0);
			break;
		case "desolate":
			displayTxt = "Produces 4 less "+conceptLink("CP")+" per "+conceptLink("economic phase")+" while colonzed (minimum 0).\
				<br /><br />A "+conceptLink("replicator")+" "+conceptLink("colony")+" at full strength does not produce a "+conceptLink("hull")+", unless its owner pays 3 CP.\
				<br /><br />"+conceptLink("AP Bot")+" colonies with this attribute instead produce an "+conceptLink("economic roll")+" <i>only</i> each tenth "+conceptLink("economic phase")+".\
				"+dspaStats(1, 0);
			break;
		case "sparta":
			displayTxt = "At the end of each "+conceptLink("economic phase")+" where there is a full strength "+conceptLink("colony")+" on this "+conceptLink("planet")+", \
				that colony produces an additional "+conceptLink("Heavy Infantry")+" or "+conceptLink("Space Marine")+" ("+conceptLink("technology")+" permitting). \
				<br /><br />Removed if colonized by a "+conceptLink("replicator")+" player."+dspaStats(0, 3);
			break;
		case "defensible":
			displayTxt = "Defending "+conceptLink("ground unit")+"s gain +1 extra "+conceptLink("Defense")+" during a gruond "+conceptLink("battle")+"."+dspaStats(4, 0);
			break;
		case "dampening":
			displayTxt = "Causes <i>all</i> ships that enter or leave this "+conceptLink("planet")+" to expend theit entire/remaining "+conceptLink("movement")+" capacity, \
				bypassing even "+conceptLink("pipeline")+" networks. A connected network still provides extra income."+dspaStats(4, 1);
			break;
		case "ambush":
			displayTxt = "Attacking ships have their "+conceptLink("weapon class")+" reduced to <b>E</b> (if not already at most that class).\
				<br /><br />Removed if colonized by a "+conceptLink("replicator")+" player."+dspaStats(4, 1);
			break;
		case "doomed":
			displayTxt = "When this "+conceptLink("planet")+" is revealed, a 6 "+conceptLink("economic phase")+" countdown begins. \
				Once the countdown reaches 0, the planet explodes; removing <i>all</i> counters from the "+conceptLink("hex")+". It is replaced with an "+conceptLink("asteroid belt")+"."+dspaStats(1, 0);
			break;
		case "builder":
			displayTxt = "Any "+conceptLink("colony")+" present may produce ships as if they had 3 extra "+conceptLink("Ship Yard")+" capacity present. \
				Removed if colonized by a "+conceptLink("replicator")+" player."+dspaStats(5, 0);
			break;
		case "spaceport":
			displayTxt = "This "+conceptLink("planet")+" can neither be colonized nor "+conceptLink("capture")+"d. Can still connect to "+conceptLink("pipeline")+" network(s). Does not prohibit "+conceptLink("battle")+"s.\
				<br /><br />Any player that has a ship present at the beginning of an "+conceptLink("economic phase")+" (assuming no cloaked intereference) may produce a single "+conceptLink("Hull Size")+" 1 ship, spending 2 extra "+conceptLink("CP")+". \
				A "+conceptLink("Replicator")+" player may instead move a newly built "+conceptLink("Hull Size")+" 1 ship to this "+conceptLink("hex")+", spending 1 "+conceptLink("CP")+".\
				"+dspaStats(0, 0);
			break;
		case "research":
			displayTxt = "The first player to colonize benefits as if they just cashed in a "+conceptLink("space wreck")+"."+dspaStats(5, 0);
			break;
		case "minor technology":
			displayTxt = "The first player to colonize this "+conceptLink("planet")+" draws 3 "+conceptLink("alien technology")+" cards instead of 2. They still keep only 1.\
				<br /><br />If a "+conceptLink("Replicator")+" player achieves this goal, they still only take and discard 1 card, gaining 10 "+conceptLink("CP")+"\
				"+dspaStats(4, 0);
			break;
		case "major technology":
			displayTxt = "The first player to colonize this "+conceptLink("planet")+" draws 3 "+conceptLink("alien technology")+" cards instead of 2. They choose <b>2</b> cards to keep.\
				<br /><br />If a "+conceptLink("Replicator")+" player achieves this goal, they instead take and discard 2 cards, gaining 20 "+conceptLink("CP")+"\
				"+dspaStats(5, 1);
			break;
		case "cloaked":
			displayTxt = conceptLink("NPA")+" ships guarding the "+conceptLink("planet")+" have "+conceptLink("Cloaking")+" 2, and will engage in battle.\
				<br /><br />Allows a "+conceptLink("Replicator")+" player to develop "+conceptLink("Scanning")+" technology for 0 CP if engaged."+dspaStats(4, 0);
			break;
		case "ranged":
		...
		case "military geniuses":
			displayTxt = conceptLink("NPA")+" ships have "+conceptLink("Attack")+" +1 / "+conceptLink("Defense")+" +1 / "+conceptLink("Tactics")+" +1. \
				The first player to colonize this draws 2 "+conceptLink("crew card")+"s, and keeps one. Crew cards can also be placed at ships orbiting this "+conceptLink("colony")+"."+dspaStats(4, 1);
			break;
		case "jedun":
			displayTxt = conceptLink("NPA")+" ships have "+conceptLink("Attack")+" +1 / "+conceptLink("Defense")+" +1 / "+conceptLink("Tactics")+" +1. \
				Ships that study at the temple for 3 consequtive "+conceptLink("turn")+"s (limit 8 "+conceptLink("Hull Size")+" worth at a time) gain these very same benefits, lasting until destroyed or "+conceptLink("capture")+"d.\
				<br /><br />Removed if colonized by a "+conceptLink("replicator")+" player, but not retroactive."+dspaStats(5, 2);
			break;
		case "telepathic":
			displayTxt = "Ships that are stationary at this "+conceptLink("colony")+" for a full turn may move to any  \
				"+conceptLink("battle")+" "+conceptLink("hex")+" they could have otherwise reached, had they moved on their turn. \
				They may also choose to move to any adjacent non-contested hex even if their colony suddenly becomes a battle hex.\
				<br /><br />Removed if colonized by a "+conceptLink("replicator")+" player."+dspaStats(5, 1);
			break;
		case "time dilation":
			displayTxt = "Time passes on this "+conceptLink("planet")+" twice as quickly, resolving 2 "+conceptLink("economic phase")+"s worth at a time; \
				but is vulnerable to 2 "+conceptLink("bombard")+"ment rolls per ship per "+conceptLink("turn")+".\
				<br /><br />A "+conceptLink("Replicator")+" "+conceptLink("colony")+" already at full strength produces 2 "+conceptLink("hull")+"s worth of ships."+dspaStats(6, 2);
			break;
		...
		case "optional rule":
			...
			break;
		case "long game":
			displayTxt = "A longer "+conceptLink("competitive")+" variant where empires are instead eliminated if they lose their "+conceptLink("homeworld")+". Last empire standing wins.\
				<br /><br />Players who eliminate one another get a +30 "+conceptLink("CP")+" bonus <i>and also</i> one level in 2 different "+conceptLink("technologies")+" where the eliminated player was superior.";
			break;
		case "short game":
			displayTxt = "A shorter "+conceptLink("competitive")+" variant where empires earn "+conceptLink("Victory Point")+"s for destroying opposing "+conceptLink("colonies")+" and having fully developed "+conceptLink("deep space")+" colonies.\
				<br /><br />If at least one player ends up with 3 "+conceptLink("VP")+" at any point, the player with the largest VP is the winner. If tied, then the largest combat fleet (in terms of "+conceptLink("CP")+") is the winner.";
			break;
		case "scenario card":
		...
		case "quick start":
			...
			if (useRuleset == "AGT") {
				displayTxt = displayTxt + "<br /><br />The rule works differently versus "+conceptLink("AP Bot")+"s than in competitive scenarios:\
					<ul style=\"max-width: 715px; margin-top: 0px; margin-left: auto; margin-right: auto; text-align: left;\">\
		...
		case "worth the effort":
			headingTxt = "Worth the Effort";
			displayTxt = conceptLink("Barren")+" "+conceptLink("colonies")+" that produced at least 1 natural "+conceptLink("CP")+" produce 2 extra CP; \
				before applying "+conceptLink("facilities")+". Incompatible with "+conceptLink("Replicators")+".";
			break;
		...
		case "no sensor lock possible":
			displayTxt = "Prevents "+conceptLink("battle")+"s from taking place inside a "+conceptLink("nebula")+". Still stops all "+conceptLink("movement")+" and prohibits "+conceptLink("retreat")+"s.";
			break;
		...
		case "advanced destroyers":
			...
			break;
		case "battlecarrier universica":
			displayTxt = conceptLink("Battle Carrier")+"s are unlocked by developing "+conceptLink("Fighters")+" 2. "+conceptLink("Replicators")+" start with an extra "+conceptLink("RP")+". Incompatible with the alternate "+conceptLink("faction")+".";
			break;
		case "advanced raiders":
			...
			break;
		case "advanced bases":
			displayTxt = "Advnaced "+conceptLink("Base")+"s are unlocked at the start. "+conceptLink("Replicators")+" start with an extra "+conceptLink("RP")+".";
			break;
		case "tough planets":
		...
		case "tough shipyards":
			displayTxt = conceptLink("Ship Yard")+"s have natural "+conceptLink("Hull Size")+" 2, regardless of "+conceptLink("empire advantage")+"s. \
				Incompatible with "+conceptLink("On The Move")+". "+conceptLink("Replicators")+" start with an extra "+conceptLink("RP")+".";
			break;
		case "stealth transports":
			...
			break;
		case "ion cannons":
			displayTxt = conceptLink("Colonies")+" provide 3 of these units temporarily (akin to "+conceptLink("militia")+") whenever their "+conceptLink("hex")+" enters a space "+conceptLink("battle")+".\
				<br />Immune to "+conceptLink("capture")+". Does not block "+conceptLink("Stealth Transports")+".";
			displayTxt = displayTxt + stats4X("Fixed", 0, "A5", 0, 1);
			break;
		case "we need the white":
			headingTxt = "We Need the White";
		...
		case "safer space":
			if (useRuleset == "AGT") {
				displayTxt = conceptLink("Scenario card")+" that weakens "+conceptLink("Danger")+" markers, causing ships to roll similarly to a "+conceptLink("black hole")+" check.";
			} else {
		...
		case "rich minerals":
			if (useRuleset == "AGT") {
				displayTxt = conceptLink("Scenario card")+" that doubles "+conceptLink("mineral")+" output when cashed in.";
			} else {
				displayTxt = "Optional rule that doubles "+conceptLink("mineral")+" output when cashed in.";
			}
		...
		case "low maintenance":
			if (useRuleset == "SE4X") {
				displayTxt = conceptLink("Optional rule")+" that causes "+conceptLink("ship")+"s to pay half as much "+conceptLink("maintenance")+" (round down total).";
					...
					displayTxt = "A benefit that can be active from a "+conceptLink("scenario card")+", "+conceptLink("Robot Race")+", Elite "+conceptLink("experience")+", and/or "+conceptLink("alien technology")+". \
						Each source cuts maintenance costs in half (relative to the previous total). If the former two sources are combined, no maintenance is paid instead.<br /><br />\
						"+conceptLink("AP Bot")+"s that benefit from this instead pay one (or more) fewer "+conceptLink("home system")+" "+conceptLink("economic roll")+"(s).";
			} else {
		...
		case "mission card":
			displayTxt = conceptLink("Resource card")+" that has a unique requirement, but provides a greater reward when completed.";
			break;
		case "where no man has gone before":
			displayTxt = "(Play during your "+conceptLink("turn")+", while there is "+conceptLink("unexplored")+" "+conceptLink("deep space")+")<br />\
				ALL ships equipped with "+conceptLink("Exploration")+" may use this ability twice each turn. \
				Awards 1 "+conceptLink("mission benefit")+" to the user of this card when all deep space is revealed." +
				discardVal(2);
			break;
		case "time travel slingshot":
			displayTxt = "(Play when one of your "+conceptLink("cruiser")+"s survives a "+conceptLink("slingshot")+")<br />\
				Leave face up. Discard to reveal all enemy ships in a single "+conceptLink("hex")+" that could be reached by one or more of your "+conceptLink("combat ship")+"s." + discardVal(2);
			break;
		case "journey to babel":
			headingTxt = "Journey to Babel";
			displayTxt = "(Play when one of your "+conceptLink("cruiser")+"s moves alone into the same "+conceptLink("hex")+" as an unconquered "+conceptLink("deep space")+" "+conceptLink("planet")+")<br />\
				It engages against a random "+conceptLink("NPA")+" ship in a one-on-one, with the latter gaining +1/+1 and all "+conceptLink("retreat")+"s disabled.<br /><br />\
				If the Cruiser wins and returns to the "+conceptLink("homeworld")+", the rest of the planet (if still unconquered) becomes allied, granting the user of the "+conceptLink("mission card")+" a 5 CP "+conceptLink("colony")+" during the next "+conceptLink("economic phase")+"." + discardVal(2);
			break;
		case "balance of terror":
			headingTxt = "Balance of Terror";
			displayTxt = "(Play during your "+conceptLink("turn")+")<br />\
				Choose one opponent. You and chosen opponent <i>each</i> sends one ship (prioritizing "+conceptLink("deep space")+" first) into "+conceptLink("battle")+", with no "+conceptLink("retreat")+"s or the use of "+conceptLink("Temporal Maneuver")+" allowed.<br /><br />\
					The winner may choose any non-homeworld "+conceptLink("colony")+" and battle/"+conceptLink("bombard")+" it. The winning ship is then returned to the original position afterwards, if it survives." + discardVal(2);
			break;
		case "creature from below":
			headingTxt = "Creature from Below";
			displayTxt = "(Play during your "+conceptLink("turn")+")<br />\
				Choose one non-homeworld "+conceptLink("colony")+". That colony gains a creature token on it. The creature must be defeated in a ground "+conceptLink("battle")+" (without the benefit of "+conceptLink("militia")+"), or it will "+conceptLink("blockade")+" the planet.<br /><br />\
					When defeated, the colony owner gains 15 extra "+conceptLink("CP")+".\
					" + stats4X("Boss", 0, "A5", 2, 4, 0) + "<br /><b>Attacks per round</b>: 3" + discardVal(2);
			break;
		case "arena":
			displayTxt = "(Play at the start of a "+conceptLink("battle")+" involving exactly one ship on each side)<br />\
				An advanced entity stops this battle, and instead assigns the captains of each ship to fight to the death.<br /><br />\
					The player that plays this card wins the battle on a roll of &le;6. The ship on the losing side is destroyed." + discardVal(2);
			break;
		case "difficult planet survey":
			displayTxt = "(Play during your "+conceptLink("turn")+")<br />\
				Choose one empty "+conceptLink("planet")+" (non-colonized and without "+conceptLink("NPA")+"s) and place a marker with matching name. \
					This planet may not be colonized, until a "+conceptLink("Cruiser")+" or "+conceptLink("Flagship")+" in the same "+conceptLink("hex")+" rolls &le;2 (limit 1 roll per turn).<br /><br />\
					The player that successfully completes this mission takes the marker and draws an extra "+conceptLink("alien technology")+" card (which they immediately keep) if they colonize it." + discardVal(2);
			break;
		case "easy planet survey":
			displayTxt = "(Play during your "+conceptLink("turn")+")<br />\
				Choose one empty "+conceptLink("planet")+" and place a marker with matching name. \
					This planet may not be colonized, until a "+conceptLink("Colony Ship")+" is accompanied by &ge;2 "+conceptLink("Scouts")+".<br /><br />\
					The first colony that is formed grows an extra step on top of the usual "+conceptLink("economic phase")+" procedures. The marker is removed once the reward is <q>consumed</q>." + discardVal(2);
			break;
		case "scientific survey":
			displayTxt = "(Play when one of your "+conceptLink("Cruiser")+"s is adjacent to a "+conceptLink("supernova")+")<br />\
				Reveal this ship. If the ship moves adjacent to 2 other supernovas, the owner passes the mission and is awarded with 1 "+conceptLink("mission benefit")+". \
				If destroyed or turned face down, the mission is failed." + discardVal(2);
			break;
		case "stellar anomaly investigation":
			displayTxt = "(Play when you have two "+conceptLink("combat ship")+"s of different types inside a "+conceptLink("deep space")+" "+conceptLink("nebula")+")<br />\
				Reveal those ships. At the end of each "+conceptLink("turn")+" while the mission is active, roll a die. On a &le;3, you pass the mission and are awarded with 1 "+conceptLink("mission benefit")+". \
				Otherwise, the mission remains incomplete. If one or both ships leave or are destroyed, the mission is failed." + discardVal(2);
			break;
		case "distress call":
			displayTxt = "(Play when one of your ships is eliminated in "+conceptLink("battle")+")<br />\
				Rather than being destroyed, place this ship in an adjacent "+conceptLink("hex")+" farther from your "+conceptLink("homeworld")+", preferring those with "+conceptLink("unexplored")+" terrain.\
				<br /><br />When explored, this ship takes the place of all terrain. It may not move until rescued by another ship of the same empire, but can defend itself if attacked." + discardVal(2);
			break;
		case "quell riots":
			displayTxt = "(Play during your "+conceptLink("turn")+")<br />\
				Choose one non-homeworld "+conceptLink("colony")+" in any "+conceptLink("home system")+". That colony gains a riot marker on it, and is considered "+conceptLink("blockade")+"d until the riot is removed.<br /><br />\
				If there are &ge;4 "+conceptLink("Hull Size")+"s worth of mobile ships in the "+conceptLink("hex")+" at the end of each "+conceptLink("turn")+", a die is rolled:\
				<br />&le;5: The riot ends, and 2 "+conceptLink("mission benefit")+"s are acquired\
				<br />&ge;9: The colony is reduced one stage, but can not be destroyed this way.\
				<br /><br />If an "+conceptLink("economic phase")+" is reached without there being enough ships to end the riot, a die is rolled per above, only without being able to end the riot." + discardVal(2);
			break;
		case "police state":
			displayTxt = "(Play during your "+conceptLink("turn")+")<br />\
				Choose one "+conceptLink("deep space")+" "+conceptLink("colony")+". That colony gains a police marker on it, and is considered permanently "+conceptLink("blockade")+"d unless the conditions below are met.<br /><br />\
				If there are &ge;4 "+conceptLink("Space Marine")+"s+"+conceptLink("Heavy Infantry")+" and &ge;6 "+conceptLink("Hull Size")+"s worth of ships revealed, the colony produces 10 "+conceptLink("CP")+" in addition to local production; regardless of any "+conceptLink("facility")+" present." + discardVal(2);
			break;
		case "new ftl test":
			headingTxt = "New FTL Test";
			displayTxt = "(Play during your "+conceptLink("turn")+")<br />\
				Choose and reveal one "+conceptLink("combat ship")+" in "+conceptLink("deep space")+" that is all alone; and has a "+conceptLink("Hull Size")+" &ge;2 and also the highest "+conceptLink("Movement")+" technology unlocked.\
				<br /><br />One die is rolled, and the results are as follows:\
				<br />&le;7: Success! The next Movement level is discounted 15 "+conceptLink("CP")+"\
				<br />8-9: Moderate failure. No long-term consequences\
				<br />10: Critical failure! The ship is destroyed!" + discardVal(2);
			break;
		case "dimensional anomaly":
			displayTxt = "(Play at the end of your exploration phase, where a "+conceptLink("cruiser")+" was your only ship that entered a "+conceptLink("hex")+" explored normally by that ship)<br />\
				That Cruiser enters another dimension, temporarily removing it from the board. At the start of each "+conceptLink("economic phase")+", roll a die. On a &le;3, it returns to the board and awards a "+conceptLink("mission benefit")+".\
				<br /><br />Otherwise, it stays in the dimension; requiring no "+conceptLink("maintenance")+", but the threshold increases by 2 each subsequent phase." + discardVal(2);
			break;
		case "asteroid strike":
			displayTxt = "(Play during your "+conceptLink("turn")+")<br />\
				Choose one "+conceptLink("asteroid belt")+" and one enemy non-homeworld "+conceptLink("colony")+" &ge;2 "+conceptLink("hex")+"es away. Place a rogue asteroid marker at the belt. This marker moves 1 hex closer to the designated colony (also given a marker) each "+conceptLink("economic phase")+".<br /><br />\
				If any player has ships with a natural "+conceptLink("Attack")+" total &ge;3, they may remove the marker. Otherwise, if the asteroid reaches the target colony, the colony is instantly destroyed." + discardVal(2);
			break;
		case "urgent deep space survey":
			displayTxt = "[Play when one of your ships (worth &ge;12 "+conceptLink("CP")+") ends its turn in a "+conceptLink("deep space")+" containing "+conceptLink("nebula")+"/"+conceptLink("asteroids")+"/"+conceptLink("black hole")+"/"+conceptLink("warp point")+"/"+conceptLink("plasma storm")+"/"+conceptLink("ion storm")+"/"+conceptLink("quantum filament")+"/"+conceptLink("quasar")+"/"+conceptLink("pulsar")+"]<br />\
				Reveal this ship. It must then ends its "+conceptLink("movement")+" in 3 more "+conceptLink("hex")+"es matching the above conditions, and then return to a friendly colony with a "+conceptLink("Ship Yard")+"/"+conceptLink("Base")+"/"+conceptLink("Starbase")+".<br /><br />\
				If succesful, claim 1 "+conceptLink("mission benefit")+". \
				If destroyed or turned face down, the mission is failed." + discardVal(2);
			break;
		case "survivors":
			displayTxt = "(Play during your "+conceptLink("turn")+")<br />\
				A shuttle containing a random enemy player's people has crash landed on a random known uncolonized "+conceptLink("NPA")+" "+conceptLink("planet")+". The planet is marked accordingly.<br /><br />\
				If the mission owner colonizes the planet, they capture the survivors and ransom them for  10 "+conceptLink("CP")+" and knowledge of "+conceptLink("technology")+". If another player colonizes it, the mission fails." + discardVal(2);
			break;
		case "sins of the father":
			headingTxt = "Sins of the Father";
			displayTxt = "(Play during your "+conceptLink("turn")+")<br />\
				Choose one of your ships with Hull Size &ge;2 in deep space. A crew member must solve a familial dispute on the "+conceptLink("homeworld")+".<br /><br />\
				The mission is successful if this ship begins and ends a turn there. If so, then a "+conceptLink("crew card")+" is drawn, usable even if the feature is otherwise disabled." + discardVal(2);
			break;
		case "defend an outpost":
			displayTxt = "(Play at the end of your "+conceptLink("turn")+")<br />\
				Choose one of your "+conceptLink("deep space")+" "+conceptLink("colonies")+". An opponent (also of your choice) must immediately assault it with 2-10 "+conceptLink("ground unit")+"s, able to be taken from <i>anywhere</i> on the board, and able to shoot immediately. No "+conceptLink("militia")+" are available.<br /><br />\
				If the attacker wins, they may "+conceptLink("capture")+" the colony as usual. If the defender wins, or the opponent chosen does not have enough ground units, then the defender claims 1 "+conceptLink("mission benefit")+"." + discardVal(2);
			break;
		case "privateers":
			displayTxt = "Choose one:<br />\
				Hire the "+conceptLink("space pirate")+" while under the benefits of "+conceptLink("Exploration")+"/"+conceptLink("regional map")+".<br />\
				OR hire this pirate even when already in control of another pirate." + discardVal(3);
			break;
		case "virus":
			displayTxt = "(Play during your "+conceptLink("turn")+")<br />\
				Choose one non-homeworld "+conceptLink("colony")+". It gains a virus counter on it, "+conceptLink("blockading")+" it. Removed at the end of the "+conceptLink("economic phase")+"." + discardVal(2);
			break;
		case "sabotage":
			displayTxt = "(Play at the beginning of your first "+conceptLink("turn")+", after a recent "+conceptLink("economic phase")+")<br />\
				Choose one opponent. That opponent <i>must</i> destroy one "+conceptLink("Defense Satellite Network")+" or any type of "+conceptLink("Base")+".<br />\
				If the Replicators are chosen, they must instead destroy one ship not in their "+conceptLink("home system")+"s." + discardVal(2);
			break;
		case "population +/-":
			displayTxt = "(Play at the end of an "+conceptLink("economic phase")+")<br />\
				Choose one non-homeworld "+conceptLink("colony")+". It grows or shrinks one stage, in the direction of the user's choice." + discardVal(2);
			break;
		case "mission benefit":
			displayTxt = "A random benefit that is awarded when a compatible "+conceptLink("mission card")+" has been completed:<br />\
				<br />1: Draw 2 "+conceptLink("resource card")+"s from an opponent's deck, and place them in your hand\
				<br />2: Choose 1 "+conceptLink("resource card")+" from ANY discard pile, and place it in your hand\
				<br />3: Look at/remotely explore any 4 "+conceptLink("unexplored")+" system markers\
				<br />4: Gain one level of "+conceptLink("Ship Size")+"\
				<br />5: Gain one level of "+conceptLink("Movement")+" technology\
				<br />6: Gain one level of "+conceptLink("Ship Yard")+"s technology\
				<br />7: Gain 10 "+conceptLink("CP")+"\
				<br />8: One "+conceptLink("Destroyer")+" (at current technology) is added to a "+conceptLink("hex")+" already containing one or more ships\
				<br />9: Place a "+conceptLink("Flagship")+" at the "+conceptLink("homeworld")+". If it already exists, upgrade it to an "+conceptLink("Advanced Flagship")+". If already fully upgraded, gain 10 "+conceptLink("CP")+"\
				<br />10: Draw 1 "+conceptLink("Alien Technology")+" card";
			break;

		// Crew cards
		case "crew card":
			displayTxt = "Individuals and roles that provides a unique power to the "+conceptLink("combat ship")+" group it is bound to. Can be killed in "+conceptLink("battle")+".<br /><br />\
				Usually, each even numbered "+conceptLink("economic phase")+"; players draw 2 cards from shared deck(s) and set aside one to be assigned \
		...
		case "wopr":
			...
			// Fall thru
		case "captain":
			displayTxt = "May reroll 1 die in the group OR 1 die used against the group per "+conceptLink("round")+".";
			if (!concept.startsWith("wopr")) {
				displayTxt = displayTxt + "<br />If on a "+conceptLink("Scout")+"/"+conceptLink("Destroyer")+"/"+conceptLink("Cruiser")+"/"+conceptLink("Missile Boat")+", \
					can reroll 2 dice per round"
			}
			break;
		case "weapons officer":
			// Fall thru
		case "mycroft":
			displayTxt = "Any natural 1s the attached group scores while shooting are scored and then rolled a <i>second</i> time.\
				<br />If on a ";
			if (concept.startsWith("Weapons")) {
				displayTxt = displayTxt + conceptLink("Scout")+"/"+conceptLink("Destroyer")+"/"+conceptLink("Raider")+"/"+conceptLink("Flagship");
			} else {
				displayTxt = displayTxt + conceptLink("Hull Size")+" 1 group";
			}
			displayTxt = displayTxt + ", the extra rolls threshold improves to &le;2.";
			break;
		...
		case "science officer":
			displayTxt = "The attached group gains the benefits of "+conceptLink("Anti-Sensor Hull")+".";
			if (!concept.startsWith("Quorra")) {
				displayTxt = displayTxt + "<br />If on a "+conceptLink("Minesweeper")+", the attached group also sweeps an extra mine per ship.";
			}
			break;
		case "marine captain":
			displayTxt = "The attached group can carry an extra "+conceptLink("ground unit")+", even on ships normally unable to carry them.\
				...
				<br />If on a "+conceptLink("Flagship")+", it may carry 4 ground units in total.";
			break;
		case "planetary admiral":
			displayTxt = "The attached group gets "+conceptLink("Attack")+" +1 if in "+conceptLink("battle")+" over a "+conceptLink("hex")+" containing a "+conceptLink("planet")+".\
				<br />If on a "+conceptLink("Battleship")+"/"+conceptLink("Dreadnought")+"/"+conceptLink("Titan")+", this bonus extends to the fleet.";
			break;
		case "admiral of the navy":
			// Fall Thru
			headingTxt = "Admiral of the Navy";
		case "agent smith":
			displayTxt = "The fleet containing the attachment gains the benefits of "+conceptLink("Expert Tacticians")+", unless the opposing fleet has access to the very same benefit.";
			break;
		case "fleet admiral":
			displayTxt = "The attached group and 2 others get "+conceptLink("Attack")+" +1 in the first round of a "+conceptLink("battle")+".\
				<br />If on a "+conceptLink("Titan")+", this bonus extends to an extra group.";
			break;
		case "skynet":
			// Fall thru
		case "admiral":
			displayTxt = "The attached group and 2 others shoot as if their "+conceptLink("weapon class")+" was one grade higher in the first round of a "+conceptLink("battle")+".";
			if (!concept.startsWith("Sky")) {
				displayTxt = displayTxt + "<br />If on a "+conceptLink("Dreadnought")+", this bonus extends to an extra group.";
			}
			break;
		case "commander":
			displayTxt = "If this fleet outnumbers the oppositon at "+conceptLink("battle")+" start, it can designate one group in the first "+conceptLink("round")+" that can not be targeted until all others are destroyed.\
				<br />If on a "+conceptLink("Battleship")+", this bonus extends to an extra group.";
			break;
		case "rear admiral":
			displayTxt = "The attached group and 1 other get "+conceptLink("Defense")+" +1 in the first round of a "+conceptLink("battle")+".\
				<br />If on a "+conceptLink("Dreadnought")+", this bonus extends to an extra group.";
			break;
		case "ash":
			// Fall thru
		case "general staff":
			displayTxt = "In each "+conceptLink("battle")+", this crew can block off one opposing "+conceptLink("retreat")+" "+conceptLink("hex")+"; \
				announced whenever they try to do so, and effective while the attached group is still afloat and in battle.";
			break;
		case "ordnance technician":
			// Fall Thru
		case "eccm officier":
			headingTxt = "Ordnance Technician / ECCM Officer";
			displayTxt = "If on a "+conceptLink("Carrier")+"/"+conceptLink("Battle Carrier")+", <i>all</i> "+conceptLink("Fighter")+"s in "+conceptLink("battle")+" that would otherwise shoot at "+conceptLink("Weapon Class")+" B instead shoot at Class A.<br /><br />\
				If on a "+conceptLink("Missile Boat")+", any "+conceptLink("missiles")+" the attached group shoots against "+conceptLink("cruisers")+" negate their "+conceptLink("jammer")+" equipment.<br /><br />\
				If on any other group, this crew may be used similarly to an "+conceptLink("Ensign Expendable")+".";
			break;
		case "warrant officer":
			// Fall Thru
		case "eccm specialist":
			headingTxt = "Warrant Officer / ECCM Specialist";
			displayTxt = "If on a "+conceptLink("Carrier")+"/"+conceptLink("Battle Carrier")+", ships in the attached group can each carry one extra "+conceptLink("Fighter")+".<br /><br />\
				If on a "+conceptLink("Missile Boat")+", the attach group can choose to figure ECCM missiles instead of regular "+conceptLink("missiles")+", allowing the negatation "+conceptLink("jammer")+" equipment (at a ratio of 2 regular missiles per 1 ECCM missile).<br /><br />\
				If on any other group, this crew may be used similarly to an "+conceptLink("Ensign Expendable")+".";
			break;
		case "squadron leader":
			displayTxt = "Ships in the attached group get "+conceptLink("Attack")+" +1. If on a "+conceptLink("Fighter")+", also "+conceptLink("Defense")+" +1 and shoots at "+conceptLink("Weapon Class")+" A regardless of terrain.";
			break;
		case "cag":
			headingTxt = "CAG";
			displayTxt = "If on a "+conceptLink("Carrier")+"/"+conceptLink("Battle Carrier")+", one group of "+conceptLink("Fighter")+"s (chosen each "+conceptLink("round")+") gets "+conceptLink("Attack")+" +1 and shoots at "+conceptLink("Weapon Class")+" A, regardless of terrain.<br /><br />\
				If on any other group, this crew may be used similarly to an "+conceptLink("Ensign Expendable")+".";
			break;
		case "deck crew chief":
			displayTxt = "If on a "+conceptLink("Carrier")+"/"+conceptLink("Battle Carrier")+", one "+conceptLink("Fighter")+" may be restored each "+conceptLink("round")+" (if any were lost during that timeframe), on a roll of &le;4/7. (For the respective ship types.)<br /><br />\
				If on any other group, this crew may be used similarly to an "+conceptLink("Ensign Expendable")+".";
			break;
		case "hal 10k":
			headingTxt = "Hal 10K";
		...
		case "gunnery officer":
			displayTxt = "Group bypasses the "+conceptLink("Weapon Class")+" penalties imposed by terrain.";
			if (!concept.startsWith("Hal")) {
				displayTxt = displayTxt + "<br />If on a "+conceptLink("Battlecruiser")+", one other group benefits from this.";
			}
			break;
		case "commodore":
			displayTxt = "At the start of "+conceptLink("battle")+", one enemy group is chosen. They may not "+conceptLink("retreat")+" until "+conceptLink("round")+" 3.\
				<br />If on a "+conceptLink("Battleship")+", this instead applies to all enemy forces.";
			break;
		case "marine commander":
			displayTxt = "If on a "+conceptLink("Transport")+", its group can drop "+conceptLink("ground unit")+"s at the start of a space "+conceptLink("battle")+" when not against a "+conceptLink("NPA")+" planet.<br /><br />\
				If on any other group, this crew may be used similarly to an "+conceptLink("Ensign Expendable")+".";
			break;
		case "marine lieutenant":
			displayTxt = "If on a "+conceptLink("Transport")+", it provides defensive support during a ground "+conceptLink("battle")+": All enemies of one chosen type get "+conceptLink("Attack")+" -2 during the first "+conceptLink("round")+".<br /><br />\
				If on any other group, this crew may be used similarly to an "+conceptLink("Ensign Expendable")+".";
			break;
		case "special warfare technician":
			displayTxt = "The attached group operates as if their "+conceptLink("Point-Defense")+"/"+conceptLink("Cloaking")+"/"+conceptLink("Scanning")+"/"+conceptLink("Minesweeping")+"/"+conceptLink("Boarding")+"/"+conceptLink("Exploration")+"/"+conceptLink("Missile Boat")+"/"+conceptLink("Jammer")+" equipment were one level higher (as compatible), even if not researched. Cannot be used to achieve a level beyond what is available at all to research.";
			break;
		case "chief engineer":
			displayTxt = "The attached group "+conceptLink("upgrade")+"s all of their equipment instantly.";
			break;
		case "patrol leader":
			displayTxt = "If the attached group spends its entire time between "+conceptLink("economic phase")+"s on "+conceptLink("pipeline")+"s connecting "+conceptLink("colonies")+",\
				an additional 3 "+conceptLink("CP")+" is generated in the next economic phase.\
				<br />If on a "+conceptLink("Destroyer")+", gain additional 5 CP instead.";
			break;
		case "astrometrics officer":
			displayTxt = "Once per "+conceptLink("economic phase")+", before new ships are placed, the owner may peek at one face down group at an adjacent "+conceptLink("hex")+" (relative to the ship containing this crew).\
				<br />If on a "+conceptLink("Cruiser")+", the range is extended to 3 hexes away from the crew instead.";
			break;
		case "operations officer":
			displayTxt = "Once per "+conceptLink("economic phase")+", the attached group may do one of the following:\
				<ul style=\"max-width: 725px; margin-top: 0px; margin-left: auto; margin-right: auto; text-align: left;\">\
				<li class=\"noKeywords\">Ignore the "+conceptLink("movement")+" effects of <b>one</b> "+conceptLink("hex")+" containing "+conceptLink("Nebula")+"/"+conceptLink("Asteroids")+"/"+conceptLink("Quantum Filament")+"/"+conceptLink("Plasma Storm")+"/"+conceptLink("Black Hole")+". If used to bypass the black hole check, a "+conceptLink("slingshot")+" may not be attempted</li>\
				<li class=\"noKeywords\">All ships in the group get "+conceptLink("Attack")+" +1 or "+conceptLink("Defense")+" +1 for one "+conceptLink("battle")+" "+conceptLink("round")+".</li></ul>\
				";
			break;
		case "hive queen":
			displayTxt = "The attached group gets "+conceptLink("Defense")+" +1, at the expense of the rest of the "+conceptLink("fleet")+" in the "+conceptLink("hex")+" getting "+conceptLink("Attack")+" +1 and "+conceptLink("Defense")+" -1.";
			break;
		case "rommie":
			headingTxt = "ROMie";
			displayTxt = "Each hit a ship in the attached group would sustain causes an extra die to be rolled. On a roll of a 1, the hit is ignored.";
			break;
		case "data":
			displayTxt = "Attached group is unaffected by "+conceptLink("asteroid")+"s/"+conceptLink("nebulae")+"/"+conceptLink("black hole")+"s when moving.";
		...
		case "marginal victory":
			headingTxt = "Marginal Victory";
			displayTxt = "A degree of victory where the winning team only <i>barely</i> achieved their objectives.";
				
			if (useRuleset == "talon") {
				displayTxt = displayTxt + "<br /><br />Also known as a minor victory. The winning team achieves this \
					if their score is less than 50% greater than the opposition.";
			}
			break;
		case "great victory":
			// Fall thru
		case "tactical victory":
			headingTxt = "Tactical Victory";
			displayTxt = "A degree of victory where the winning team achieves their objectives with a moderate success.";
				
			if (useRuleset == "talon") {
				displayTxt = displayTxt + "<br /><br />Also known as a great victory. The winning team achieves this \
					if their score is 50-199% greater than the opposition.";
			}
			break;
		case "devastating victory":
			// Fall thru
		case "decisive victory":
			headingTxt = "Decisive Victory";
			displayTxt = "A degree of victory where the winning team achieves their objectives with an overwhelming success.";
				
			if (useRuleset == "talon") {
				displayTxt = displayTxt + "<br /><br />Also known as a devastating victory. The winning team achieves this \
					if their score is at least 200% greater than the opposition.";
			}
			break;
		case "empire war":
			...
			break;
		case "conflict zone":
			displayTxt = "An active zone in a given "+conceptLink("Empre War")+" sector where the next "+conceptLink("battle")+" would take place (for the sector). A team conquers this zone if:\
				<ol style=\"max-width: 350px; margin-top: 0px; margin-left: auto; margin-right: auto; text-align: left;\">\
				<li class=\"noKeywords\">They win at least a "+conceptLink("tactical victory")+"</li>\
				<li class=\"noKeywords\">They win without losing any ships, or...</li>\
				<li class=\"noKeywords\">Their opposition decides to <i>not</i> engage this "+conceptLink("battle")+"</li></ol>\
				If a battle takes place over a "+conceptLink("planet")+", the invading team must also be able to achieve "+conceptLink("Planetary Invasion")+"'s unique objective.";
			break;
		case "sp":
		...
		case "reserve":
			if (useRuleset == "talon") {
				...
				displayTxt = "Resources that are set aside, for later use.";
			}
			break;
		case "spool":
			headingTxt = "Spooling";
			displayTxt = "A process by which a mobile ship recharges its FTL drive. A ship must be on the board for as many "+conceptLink("round")+"s as its "+conceptLink("hull")+" total \
				to be eligible to "+conceptLink("retreat")+" from "+conceptLink("battle")+" this way. \
				It also must be in open space, and must not have sustained the "+conceptLink("FTL Offline")+" condition.";
			break;
		...
		case "escort zone":
			displayTxt = "Simple "+conceptLink("Talon")+" skirmish scenario (limited to 200 "+conceptLink("SP")+" each) \
				where each faction has a "+conceptLink("Transport")+" as their"+conceptLink("primary objective")+".<br />For scoreboard purposes, its escape or destruction counts <i>double</i> the normal SP.";
			break;
		...
		case "priority target":
			...
			displayTxt = "Concept where a designated "+conceptLink("AI")+" ship moves almost exclusively towards its designated target \
		...
		case "center hex row":
			displayTxt = "The "+conceptLink("hex")+"es directly in front of, and directly behind, a given "+conceptLink("ship")+" constitute that ship's <q>center hex row</q>.\
				<br />This row changes whenever the ship "+conceptLink("turn")+"s, "+conceptLink("side slip")+"s, or is displaced involuntarily outside the row.";
			break;
		case "impulse":
			displayTxt = "A small segment of time. The actions a given "+conceptLink("ship")+" can do is based on how much time has passed in a given "+conceptLink("round")+".<br />\
				...
				It can use its "+conceptLink("battery")+" or "+conceptLink("afterburner")+"s to gain the respective resources whenever it would otherwise get none.<br />\
				A ship with one or more fully charged weapon banks can also shoot them, even if it did none of the previous actions.<br />\
				<table><caption>Action Usage</caption>\
				<tr><td colspan=\"2\" rowspan=\"2\"></td><th colspan=\"6\">Impulse</th></tr>\
				<tr><th>A</th><th>B</th><th>C</th><th>D</th><th>E</th><th>F</th></tr>\
				<tr><th rowspan=\"6\">R<br />a<br />t<br />i<br />n<br />g</th><th>1</th>\
				<td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td class=\"activePhase\">&nbsp;</td></tr>\
				<tr><th>2</th>\
				<td>&nbsp;</td><td>&nbsp;</td><td class=\"activePhase\">&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td class=\"activePhase\">&nbsp;</td></tr>\
				<tr><th>3</th>\
				<td>&nbsp;</td><td class=\"activePhase\">&nbsp;</td><td>&nbsp;</td><td class=\"activePhase\">&nbsp;</td><td>&nbsp;</td><td class=\"activePhase\">&nbsp;</td></tr>\
				<tr><th>4</th>\
				<td class=\"activePhase\">&nbsp;</td><td>&nbsp;</td><td class=\"activePhase\">&nbsp;</td><td class=\"activePhase\">&nbsp;</td><td>&nbsp;</td><td class=\"activePhase\">&nbsp;</td></tr>\
				<tr><th>5</th>\
				<td class=\"activePhase\">&nbsp;</td><td class=\"activePhase\">&nbsp;</td><td class=\"activePhase\">&nbsp;</td><td class=\"activePhase\">&nbsp;</td><td>&nbsp;</td><td class=\"activePhase\">&nbsp;</td></tr>\
				<tr><th>6</th>\
				<td class=\"activePhase\">&nbsp;</td><td class=\"activePhase\">&nbsp;</td><td class=\"activePhase\">&nbsp;</td><td class=\"activePhase\">&nbsp;</td><td class=\"activePhase\">&nbsp;</td><td class=\"activePhase\">&nbsp;</td></tr>\
				</table>";
			break;
		case "collide":
			// Fall thru
		...
		case "dock":
			headingTxt = "Docking";
			displayTxt = conceptLink("Fighter")+" squadrons can dock with a "+conceptLink("carrier")+", if they intend to "+conceptLink("retreat")+" together.<br /><br />\
				Some scenarios allow "+conceptLink("ship")+"s to dock with a "+conceptLink("base")+". Either way; this process is permanent, unless specified otherwise.";
			break;
		case "power":
			displayTxt = "Energy that is not spent to move a "+conceptLink("ship")+".<br />\
				...
				Available actions:<ol type=\"A\" style=\"max-width: 650px; margin-top: 0px; margin-left: auto; margin-right: auto; text-align: left;\">\
				...
				<li class=\"noKeywords\">Transfer power to a "+conceptLink("Fighter")+" squad, allowing them to perform A/B/C<br />\
				(only available to "+conceptLink("Base")+"s and "+conceptLink("Carrier")+"s within 6 "+conceptLink("hex")+"es)</li>\
				<li class=\"noKeywords\">Selecting a new target for <i>1</i> "+conceptLink("missile")+"\
				(only available to ships with intact "+conceptLink("Missile Launcher")+"s)</li></ol>";
			break;
		case "weapon group":
			headingTxt = "Weapon Group";
			displayTxt = "One or more weapons that shoot and recharge as a single bank. Can also be destroyed as a group, as part of a "+conceptLink("critical")+" roll.<br /><br />\
				Weapons are divided into red segments (time), and yellow segments (energy). Larger groups need more energy when actively charging from "+conceptLink("power")+", but also passively recharge faster during a "+conceptLink("power phase")+" <span class=\"bindTxt\">(1 segment per weapon)</span>; if all red segments in the group are already full.";
			break;
		case "batteries":
			// Fall thru
		case "battery":
		...
		case "turn radius":
			...
			break;
		case "power phase":
			displayTxt = "A regrouping phase after each 6 "+conceptLink("impulse")+"s where "+conceptLink("ship")+"s can recharge their "+conceptLink("weapon group")+"s and adjust their "+conceptLink("power curve")+" by one stage.\
		...
		case "shield":
			headingTxt = "Shields";
			displayTxt = "A layer of protection that "+conceptLink("ship")+"s use to absorb damage, divided into 4 quadrants. Once a given quadrant is down, then the "+conceptLink("hull")+" starts taking damage from that direction.\
				<br /><br />These layers can be reinforced with "+conceptLink("power")+", or the entire subsystem can be temporarily rendered unusable via contact with "+conceptLink("nebula")+", or the "+conceptLink("Shields Down")+" condition.";
			break;
		...
		case "wave-motion gun":
			...
			displayTxt = "Expert "+conceptLink("Terran")+" weapon that displaces the victim 1 "+conceptLink("hex")+" <i>and</i> deals 10 damage, but is very power intensive (2R5Y).\
		...
		case "missile launcher":
			...
			break;
		case "fusion cannon":
			displayTxt = "Expert "+conceptLink("Talon")+" weapon that damages ALL targets within the firing arc, but reloads extremely slowly (3R3Y). \
				"+conceptLink("Fighter")+"s are immune, but not "+conceptLink("missile")+"s.\
		...
		case "secondary explosion": // 2
			...
			break;
		case "shields down": // 3
			displayTxt = conceptLink("Ship")+"s that have this condition are unable to use their "+conceptLink("shield")+"s. Temporarily reinforced shields are also negated. "+conceptLink("Repairable");
			break;
		case "helm down": // 4
			displayTxt = conceptLink("Ship")+"s that have this condition are unable to "+conceptLink("turn")+" or "+conceptLink("side slip")+". No effect on "+conceptLink("base")+"s and "+conceptLink("starbase")+"s. "+conceptLink("Repairable");
			break;
		case "random weapon group destroyed": // 5 or 9
			displayTxt = "Each time a "+conceptLink("ship")+" sustains this condition, one of their "+conceptLink("weapon group")+"s has been permanently destroyed.";
			break;
		case "manuevering thruster damage": // 6
			headingTxt = "Manuevering Thruster Damage +X";
			displayTxt = conceptLink("Ship")+"s that have this condition have a permanently increased "+conceptLink("turn radius")+" corresponding to the indicated amount, effective the next time the ship "+conceptLink("turn")+"s. Maxes out at +2. \
				No effect on "+conceptLink("base")+"s and "+conceptLink("starbase")+"s";
			break;
		case "power relay damage": // 8
			headingTxt = "Power Relay Damage -X";
			displayTxt = conceptLink("Ship")+"s that have this condition have a further reduced "+conceptLink("power")+" output, starting with the next "+conceptLink("power phase")+". \
				Maxes out at -2, but stacks with any penalties imposed by "+conceptLink("hull")+" damage.<br/>"+conceptLink("AI")+" ships that roll this condition instead take 1 additional "+conceptLink("hull")+" damage.";
			break;
		case "ftl offline": // 10. Also adds 1 hull dmg
			headingTxt = "FTL Offline";
			displayTxt = conceptLink("Ship")+"s that have this condition are unable to "+conceptLink("spool")+"/use their FTL drive. Repaired at the end of "+conceptLink("battle")+".";
			break;
		case "power loss": // 11
			displayTxt = conceptLink("Ship")+"s that have this condition are unable to receive "+conceptLink("power")+" from any source, \
				nor adjust their "+conceptLink("power curve")+" or recharge weapons. "+conceptLink("Repairable")+"\
		...
		case "ftl core breach": // 12
			...
			break;
		case "repairable":
			headingTxt = "Repairable Damage";
			displayTxt = "Some "+conceptLink("critical")+" damage can be repaired in the middle of a "+conceptLink("battle")+". Starting with the "+conceptLink("impulse")+" after the damage was inflicted, 1d6 is rolled. On a roll of a 6, the damage is repaired. Damage repairable this way is also repaired at the end of the battle.";
			break;
		case "explode":
			// Double fall thru
		case "exploding":
		case "explosion":
			...
			displayTxt = "A "+conceptLink("ship")+" that loses all of its "+conceptLink("hull")+" points <i>or</i> suffers a meltdown with their FTL core explodes, \
				...
				If multiple small craft are in an affected hex, only one of them is damaged in each hex.";
			break;
		...
		case "destroyer-g":
			...
			displayTxt = "Faction-exclusive variant that carries extra guided missiles<br />";
			displayTxt = displayTxt + statsTalon("Talon", 69, "Missile Launcher x3", "5/4/4/3", 4, 2, 2);
			break;
		case "destroyer-p":
			headingTxt = "Destroyer-P (DDP)";
			displayTxt = "Faction-exclusive variant that uses only "+conceptLink("Phaser")+"s. Rivals with the "+conceptLink("Destroyer-D")+"<br />";
		...
		case "battle simulator":
			displayTxt = "Program that simulates "+conceptLink("battle")+"s using its internal dice rolling and targeting systems.<br />\
				A limited subset of solo scenarios are supported.";
			provideLinks = 1;
			break;
		case "unique designer":
			displayTxt = "Program that allows designing/sharing "+conceptLink("Unique Ship")+" configurations.";
			provideLinks = 2;
			break;
		case "fleet builder":
			...
			break;
		case "shuffler":
			displayTxt = "Programs that can shuffle and deal <i>nearly any</i> mix of appropriate elements. Nearly any set of randomizable elements are supported.\
		...
		case "numsims":
			...
			break;
		case "transcreening":
			headingTxt = "Transport Screening";
			displayTxt = "If "+conceptLink("transport")+"s are brought into battle, they typically are "+conceptLink("screen")+"ed (if able) for strategic reasons.\
				<br />For example; against the "+conceptLink("replicators")+", keeping them alive is a good idea to dismantle their "+conceptLink("colonies")+".";
			break;
		case "retreatthresh":
		...
		case "ftrpresence":
			...
			break;
		case "designsize":
			headingTxt = "Minimum Ship Size";
			displayTxt = "The "+conceptLink("Ship Size")+" level that must be reached to be able to build this ship. Thresholds are based on the "+conceptLink("CP")+" cost to build ships for the regular "+conceptLink("faction")+".";
			break;
		case "designwep":
			headingTxt = "Minimum Weapon Class";
			displayTxt = "The lowest "+conceptLink("Weapon Class")+" that must be equipped on this design. Each 10 "+conceptLink("CP")+" in cost forces the minimum class to rise one tier.";
			break;
	...
	switch (provideLinks) {
		...
		case 4:
			if (useRuleset == "talon") {
				displayTxt = displayTxt + "<a class=\"interact\" href=\"/se4x/shufflers/talonAI.htm\" target=\"_blank\">Talon AI</a>";
			} else {
				displayTxt = displayTxt + "<a class=\"interact\" href=\"/se4x/shufflers/homeSystem.htm\" target=\"_blank\">Home System</a>\
					<a class=\"interact\" href=\"/se4x/shufflers/deepSpace.htm\" target=\"_blank\">Deep Space</a>\
					<a class=\"interact\" href=\"/se4x/shufflers/npa.htm\" target=\"_blank\">NPA</a>";
				
				if (useRuleset != "SE4X") {
					displayTxt = displayTxt + "<a class=\"interact\" href=\"/se4x/shufflers/empireAdv.htm\" target=\"_blank\">Empire Advantage</a>\
						<a class=\"interact\" href=\"/se4x/shufflers/alienTech.htm\" target=\"_blank\">Alien Tech</a>";
					
					if (useRuleset != "CE") {
						displayTxt = displayTxt + "<a class=\"interact\" href=\"/se4x/shufflers/resource.htm\" target=\"_blank\">Resource Cards</a>";
						
						if (useRuleset != "Rep") {
							displayTxt = displayTxt + " <span class=\"bindTxt\"><a class=\"interact\" href=\"/se4x/shufflers/auxTech.htm\" target=\"_blank\">Aux Tech</a>\
								<a class=\"interact\" href=\"/se4x/shufflers/crew.htm\" target=\"_blank\">Crew</a>\
								<a class=\"interact\" href=\"/se4x/shufflers/scenario.htm\" target=\"_blank\">Scenario Cards</a>\
								<a class=\"interact\" href=\"/se4x/shufflers/dspa.htm\" target=\"_blank\">DSPA</a></span>";
						}
					}
				}
			}
...
function showCards(plrName, label, cardArray) {
	var infoPanel = document.getElementById("infobox");
	var displayTxt = "";

	infoPanel.style.maxWidth = "500px";
	
	displayTxt = "<b class=\"headOx\">"+label+" for "+plrName+"</b><br />";
...
function showSpecsTalon(namee, pwrCurve, shields, wepDetails, hullDmg, critDmg, auxFeat) {
	...
	if (shields.length >= 4) {
		displayTxt = displayTxt + "<table class=\"clean\" style=\"margin-top: 0px; margin-bottom: 0px;\">\
			<tr><td rowspan=\"2\"><b><a href=\"javascript:showBox('shield')\">Shields</a></b>:</td>\
			<td rowspan=\"2\">"+shields[1]+"</td><td>"+shields[0]+"</td><td rowspan=\"2\">"+shields[2]+"</td></tr>\
			<tr><td>"+shields[3]+"</td></tr></table>";
	}
...
function keywordifyCollection(collObj) {
	const keyTerms = ["Space Empires 4X", "Close Encounters", "Replicator", "All Good Things", "AGT", "Space Imperia 4X", "Space Empires Anthology",
		...
		"AI", "Barren", "Campaign", "Colony", "Colonies", "Combat Ship", "CP", "Economic Phase", "EP", "Faction", "Homeworld", "Maintenance", "Planet", "Starship", "Scuttle", "Starting Forces", "Turn",
		"Bid", "Competitive", "Cooperative", "Galactic Capitol", "Initiative", "Primary Objective", "Public Information", "Free for All", "Team Game", "Uneasy Alliance", "Versus Map", "Victory Point", "VP", "Blood Brothers",
		"Battle", "Battling", "Blockade", "Blockading", "Bombard", "Fleet", "FSB", "Immobile", "Non-Player Alien", "NPA", "Subdue", "Subduing", "Priority Class", "Retreat", "Round", "Screen", "Weakness", "Weapon Class",
		"Alien-E", "Alien-D", "Alien-C", "Alien-B", "Alien-A", "Doomsday Machine", "DM", "Amoeba", "Alien Empires", "AP", "Alien Player", "Counter Tech", "Economic Roll",
		...
		"Warp Point", "Regional Map", "Fold in Space", "Folds in Space", "Pirate",
		"Technology", "Technologies", "Attack", "Defense", "Explore", "Exploration", "Movement", "PD", "React Move", "Ship Size", "Tactics", "Terraforming", "Upgrade", "Upgrading",
		"Cloaking", "Fighter", "Minelaying", "Minesweeping", "Nanomachine", "Scanning",
		"Fastmove", "Second Salvo",
		"Quick Start", "Short Game", "Long Game", "Slingshot", "Gearing Limits", "Unpredictable Research", "Research Grant", "Heavy Terrain",
		"Safer Space", "Slow Scientists", "Smart Scientists", "Bloody Combat", "Head Start", "Galactic Situation",
		"Transport", "Troops", "Capture", "Capturing", "Capturable", "Militia", "Light Infantry", "Space Marines", "Heavy Infantry", "Grav Armor", "Drop Ships",
		...
		"Type Exp", "Type Flag", "Type Scan", "Type SW",
		"Cosmic Storm", "Ion Storm", "Plasma Storm", "Pulsar", "Quantum", "Quasar", "Sinister", "Space Station",
		...
		"Frigate", "Phaser", "Anti-Matter Torpedo", "Wave-Motion Gun", "Disruptor", "Fusion Cannon", "Laser", "Cobalt Cannon",
	...
	const keyExpressions = [
		...
		{regex: conceptLink("Hull")+" Size", newTxt: conceptLink("Hull Size")},
		{regex: "M"+conceptLink("VP"), newTxt: conceptLink("MVP")},
		...
		{regex: conceptLink("base")+"line", newTxt: "baseline"},
		...
		{regex: conceptLink("Planet")+"ary "+conceptLink("Bombard")+"ment", newTxt: conceptLink("Planetary Bombardment")},
		{regex: "Discover Member of "+conceptLink("Ancient Race"), newTxt: conceptLink("Discover Member of Ancient Race")},
		{regex: conceptLink("Mineral")+"s +5/-3", newTxt: conceptLink("Minerals +5/-3")},
		{regex: "Hidden "+conceptLink("Power"), newTxt: conceptLink("Hidden Power")},
		{regex: "Easy "+conceptLink("Planet")+" Survey", newTxt: conceptLink("Easy Planet Survey")},
		{regex: conceptLink("Asteroid")+" Strike", newTxt: conceptLink("Asteroid Strike")},
		{regex: "Urgent "+conceptLink("Deep Space")+" Survey", newTxt: conceptLink("Urgent Deep Space Survey")},
		{regex: "Engine "+conceptLink("Sabotage"), newTxt: conceptLink("Engine Sabotage")},
		{regex: "Anti-"+conceptLink("Replicator"), newTxt: conceptLink("Anti-Replicator")},
		{regex: "anti-"+conceptLink("replicator"), newTxt: conceptLink("anti-replicator")},
		{regex: conceptLink("battle")+" "+conceptLink("carrier"), newTxt: conceptLink("battle carrier")},
		{regex: conceptLink("Battle")+" "+conceptLink("Carrier"), newTxt: conceptLink("Battle Carrier")},
		{regex: "Type "+conceptLink("PD"), newTxt: conceptLink("Type PD")},
		...
		{regex: conceptLink("Coup")+"le", newTxt: "Couple"},
		{regex: conceptLink("coup")+"le", newTxt: "couple"},
		...
		{regex: "storm "+conceptLink("movement"), newTxt: conceptLink("storm movement")},
		{regex: "Advanced "+conceptLink("Destroyer")+"s", newTxt: conceptLink("Advanced Destroyers")},
		{regex: "advanced "+conceptLink("destroyer")+"s", newTxt: conceptLink("advanced destroyers")},
		{regex: "Auxiliary "+conceptLink("Technology"), newTxt: conceptLink("Auxiliary Technology")},
		{regex: "auxiliary "+conceptLink("technology"), newTxt: conceptLink("auxiliary technology")},
		{regex: "Hidden "+conceptLink("Exploration"), newTxt: conceptLink("Hidden Exploration")},
		{regex: "hidden "+conceptLink("exploration"), newTxt: conceptLink("hidden exploration")},
		{regex: conceptLink("AP")+" Bot", newTxt: conceptLink("AP Bot")},
		{regex: conceptLink("AP")+" bot", newTxt: conceptLink("AP bot")},
		...
		{regex: "heavy "+conceptLink("cruiser"), newTxt: conceptLink("heavy cruiser")},
		{regex: conceptLink("Fleet")+" "+conceptLink("Builder"), newTxt: conceptLink("Fleet Builder")},
		{regex: conceptLink("fleet")+" "+conceptLink("builder"), newTxt: conceptLink("fleet builder")},
		{regex: "Non-"+conceptLink("critical"), newTxt: "Non-critical"},
		{regex: "non-"+conceptLink("critical"), newTxt: "non-critical"},
		{regex: "Random "+conceptLink("Weapon Group")+" Destroyed", newTxt: conceptLink("Random Weapon Group Destroyed")},
		{regex: "random "+conceptLink("Weapon group")+" destroyed", newTxt: conceptLink("random weapon group destroyed")},
		{regex: conceptLink("Turn")+" Radius", newTxt: conceptLink("Turn Radius")},
		{regex: conceptLink("turn")+" radius", newTxt: conceptLink("turn radius")},
		{regex: conceptLink("Turn")+" Radii", newTxt: conceptLink("Turn Radii")},
		{regex: conceptLink("turn")+" radii", newTxt: conceptLink("turn radii")},
		{regex: conceptLink("Power")+" Curve", newTxt: conceptLink("Power Curve")},
		{regex: conceptLink("power")+" curve", newTxt: conceptLink("power curve")},
		{regex: conceptLink("Power")+" Phase", newTxt: conceptLink("Power Phase")},
		{regex: conceptLink("power")+" phase", newTxt: conceptLink("power phase")},
		{regex: "p"+conceptLink("reserve"), newTxt: "preserve"},
		{regex: "fire"+conceptLink("power"), newTxt: "firepower"},
		...
		{regex: "Worm Hole "+conceptLink("Blockade"), newTxt: conceptLink("Worm Hole Blockade")},
		...
		{regex: "energy "+conceptLink("nebula"), newTxt: conceptLink("energy nebula")},
		{regex: conceptLink("scout")+"-e", newTxt: conceptLink("scout-e")},
		{regex: conceptLink("Scout")+"-E", newTxt: conceptLink("Scout-E")},
		{regex: conceptLink("frigate")+"-e", newTxt: conceptLink("frigate-e")},
		{regex: conceptLink("Frigate")+"-E", newTxt: conceptLink("Frigate-E")},
		...
		{regex: conceptLink("destroyer")+"-x", newTxt: conceptLink("destroyer-x")},
		{regex: conceptLink("Light Cruiser")+"-X", newTxt: conceptLink("Light Cruiser-X")},
		{regex: conceptLink("light cruiser")+"-x", newTxt: conceptLink("light cruiser-x")},
		{regex: conceptLink("Heavy Cruiser")+"-X", newTxt: conceptLink("Heavy Cruiser-X")},
		{regex: conceptLink("heavy cruiser")+"-x", newTxt: conceptLink("heavy cruiser-x")},
		{regex: conceptLink("Battlecruiser")+"-H", newTxt: conceptLink("Battlecruiser-H")},
		{regex: conceptLink("battlecruiser")+"-h", newTxt: conceptLink("battlecruiser-h")},
